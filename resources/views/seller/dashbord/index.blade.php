  ...
  <div class="row">
    ...
    <div class="col-lg-12 col-xl-12">
      ...
      <div class="card mb-3">
        ...
        <div class="card-body">
          <h5><b>{{ __("Plan Name:") }}</b>
            {{ !empty(auth()->user()->activeSubscription->plan) ? auth()->user()->activeSubscription->plan->name : __("seller.Plan not found !") }}
          </h5>
          <h5><b>{{ __("Product Upload Limit:") }}</b>
            {{ !empty(auth()->user()->activeSubscription->plan) ? auth()->user()->products()->count().' / '.auth()->user()->activeSubscription->plan->product_create : __("seller.Plan not found !") }}
          </h5>
          <h5><b>{{ __("Expires ON:") }}</b>
            {{ !empty(auth()->user()->activeSubscription) ?  date('d/m/Y h:i A',strtotime(auth()->user()->activeSubscription->end_date)) : __("seller.Not found !")}}
          </h5>
          <h5><b>{{ __("CSV Product Upload:") }}</b>
            {{ !empty(auth()->user()->activeSubscription->plan) && auth()->user()->activeSubscription->plan->csv_product ? __("seller.YES")  : __("seller.NO")}}
          </h5>
        </div>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td><a title="View order"
                          href="{{ route('seller.view.order',!empty($order->order_id) ? $order->order_id : '') }}">#{{ !empty($order->order_id) ? $inv_cus->order_prefix.$order->order_id : '' }}</a>
                      </td>
